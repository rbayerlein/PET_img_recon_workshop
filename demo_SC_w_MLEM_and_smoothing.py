...
num_ct_scaled = y_sct_scaled.sum()
y_sct_smooth = gaussian_filter(y_sct_scaled, sigma)
num_ct_smoothed = y_sct_smooth.sum()
y_sct_smooth *= num_ct_scaled/num_ct_smoothed # normalization step
recon_SC_est = mlem(y, n_iter, S_AC, T, r_sino=y_rnd, s_sino=y_sct_smooth)
...
im2 = ax[0,2].imshow(y_sct_smooth, cmap='magma', aspect='auto');     ax[0,2].set_title("Scaled & smoothed scatter sino (MC)"); plt.colorbar(im2, ax=ax[0,2])
