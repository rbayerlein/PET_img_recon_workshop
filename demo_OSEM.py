...
# definistion of OSEM subsets
subsets = [np.arange(i, len(angles), num_subsets) for i in range(num_subsets)]
def A_sub(x, th):  return radon(x, theta=angles[th], circle=True, preserve_range=True)
def AT_sub(y, th): return iradon(y, theta=angles[th], filter_name=None, circle=True,
                                 output_size=img.shape[0], preserve_range=True)

def osem(y, n_iter=20):
    ...
    for k in range(n_iter):
        for th in subsets:
            Ax = A_sub(x, th) + eps         # forward projection
            ratio = y[:, th] / np.clip(Ax, eps, None)           # update term  
            x *= AT_sub(ratio, th) / sens   # back projection
...
recon_exmpl  = osem(y_proj_data, num_iter)
...
ax[1].set_title(f"MLEM ({num_iter} iterations, {num_subsets} subsets)"); ax[1].axis('off')
